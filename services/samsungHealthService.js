// 삼성 헬스 연동 서비스 - HealthKit과 동일한 인터페이스 제공
import { Platform, NativeModules } from 'react-native';
import env from '../config/environment';

// Samsung Health 네이티브 모듈 로드
async function loadSamsungHealthModule() {
  // Android에서만 로드
  if (Platform.OS !== 'android') return undefined;
  
  try {
    const native = NativeModules?.SamsungHealth;
    if (native && (typeof native.isAvailable === 'function' || typeof native.checkPermissions === 'function')) {
      return native;
    }
  } catch (e) {
    console.warn('⚠️ Samsung Health 모듈 로드 실패:', e);
  }
  
  console.warn('⚠️ Samsung Health 모듈 로드 실패: NativeModules keys=', Object.keys(NativeModules || {}));
  return undefined;
}
...
class SamsungHealthService {
  ...
  async initialize() {
    try {
      console.log('🏥 Samsung Health 초기화 시작');
      
      // 플랫폼 가드
      if (Platform.OS !== 'android') {
        console.warn('⚠️ Android 이외 플랫폼에서는 Samsung Health를 사용할 수 없습니다.');
        this.isAvailable = false;
        this.isInitialized = false;
        return false;
      }
      
      const SamsungHealth = await loadSamsungHealthModule();
      
      // 네이티브 모듈 가드
      if (!SamsungHealth) {
        console.warn('⚠️ Samsung Health 네이티브 모듈이 로드되지 않았습니다.');
        this.isAvailable = false;
        this.isInitialized = false;
        return false;
      }
      
      if (typeof SamsungHealth.isAvailable !== 'function') {
        console.warn('⚠️ Samsung Health 메서드가 없습니다.');
        this.isAvailable = false;
        this.isInitialized = false;
        return false;
      }
      ...
      this.isAvailable = await SamsungHealth.isAvailable();
      
      if (this.isAvailable) {
        // 권한 요청
        const hasPermission = await SamsungHealth.requestPermissions();
        if (hasPermission) {
          this.isInitialized = true;
          console.log('✅ Samsung Health 초기화 성공');
          return true;
        } else {
          console.warn('⚠️ Samsung Health 권한이 거부되었습니다.');
          this.isInitialized = false;
          return false;
        }
      } else {
        console.log('❌ Samsung Health를 사용할 수 없습니다.');
        return false;
      }
    } catch (error) {
  ...
  async checkPermissions() {
    try {
      console.log('🔍 Samsung Health 사용 가능 여부 확인');
      
      if (Platform.OS !== 'android') {
        return {
          isAvailable: false,
          hasPermissions: false,
          error: '비 Android 환경에서는 Samsung Health를 사용할 수 없습니다.'
        };
      }
      ...
      // 네이티브 모듈 가드
      if (!SamsungHealth || typeof SamsungHealth.checkPermissions !== 'function') {
        return {
          isAvailable: false,
          hasPermissions: false,
          error: 'Samsung Health 네이티브 모듈이 연결되지 않았습니다.'
        };
      }
      
      // Samsung Health 사용 가능 여부 확인
      const isAvailable = await SamsungHealth.isAvailable();
      
      if (!isAvailable) {
        return {
          isAvailable: false,
          hasPermissions: false,
          error: 'Samsung Health를 사용할 수 없습니다.'
        };
      }
      ...
      // 권한 확인
      const permissionStatus = await SamsungHealth.checkPermissions();
      ...
      console.error('❌ Samsung Health 상태 확인 실패:', error);
  ...
  async requestPermissions() {
    try {
      console.log('🔍 Samsung Health 권한 요청 시작');
      ...
      // 모듈 로드 및 가드
      const SamsungHealth = await loadSamsungHealthModule();
      if (!SamsungHealth || typeof SamsungHealth.requestPermissions !== 'function') {
        console.warn('⚠️ Samsung Health 네이티브 모듈이 연결되지 않았습니다.');
        return false;
      }
      ...
      if (granted) {
        console.log('✅ Samsung Health 권한 요청 성공');
        this.isInitialized = true;
        this.isAvailable = true;
        return true;
      } else {
        console.warn('⚠️ Samsung Health 권한 요청 실패');
        return false;
      }
    } catch (error) {
      console.error('❌ Samsung Health 권한 요청 실패:', error);
      return false;
    }
  }
  ...
  async getRouteCoordinates(startDate, endDate) {
    try {
      if (!this.isServiceAvailable()) {
        console.warn('⚠️ Samsung Health 서비스를 사용할 수 없습니다.');
        return [];
      }

      const SamsungHealth = await loadSamsungHealthModule();
      if (!SamsungHealth || typeof SamsungHealth.getWorkoutRouteSamples !== 'function') {
  ...
  async findMatchingWorkout(event) {
    try {
      console.log('🔍 [SamsungHealthService] findMatchingWorkout 시작');
      console.log('🔍 [SamsungHealthService] event:', JSON.stringify(event, null, 2));
      
      // 이벤트 시간 파싱
      console.log('🔍 [SamsungHealthService] 이벤트 시간 파싱 시작');
      const eventTime = this.parseEventTime(event);
      if (!eventTime) {
        console.warn('⚠️ [SamsungHealthService] 이벤트 시간을 파싱할 수 없습니다.');
        return null;
      }
      console.log('✅ [SamsungHealthService] 이벤트 시간 파싱 완료:', eventTime.toISOString());

      // Samsung Health 서비스 사용 가능 여부 확인
      console.log('🔍 [SamsungHealthService] Samsung Health 서비스 사용 가능 여부 확인');
      const isAvailable = this.isServiceAvailable();
      console.log('🔍 [SamsungHealthService] isServiceAvailable 결과:', isAvailable);
      
      if (!isAvailable) {
        console.warn('⚠️ [SamsungHealthService] Samsung Health 서비스를 사용할 수 없습니다.');
        console.warn('⚠️ [SamsungHealthService] 초기화 시도...');
        const initialized = await this.initialize();
        console.log('🔍 [SamsungHealthService] 초기화 결과:', initialized);
        
        if (!initialized) {
          console.warn('⚠️ [SamsungHealthService] Samsung Health 초기화 실패');
          return null;
        }
      }

      console.log('🔍 [SamsungHealthService] Samsung Health 모듈 로드 시작');
      const SamsungHealth = await loadSamsungHealthModule();
      console.log('🔍 [SamsungHealthService] Samsung Health 모듈 로드 완료:', SamsungHealth ? '성공' : '실패');
      
      if (!SamsungHealth || typeof SamsungHealth.getSamples !== 'function') {
        console.warn('⚠️ [SamsungHealthService] Samsung Health 모듈을 사용할 수 없습니다.');
        return null;
      }
      console.log('✅ [SamsungHealthService] Samsung Health 모듈 사용 가능');
      ...
      console.log('🔍 [SamsungHealthService] 워크아웃 조회 시작:', {
        이벤트시간: {
      ...
      // 워크아웃 데이터 조회
      const workouts = await SamsungHealth.getSamples({
        startDate: startDateISO,
        endDate: endDateISO,
        type: 'Workout'
      });

      console.log('🔍 [SamsungHealthService] Samsung Health 조회 완료');
      
      if (!workouts || workouts.length === 0) {
        console.log('❌ [SamsungHealthService] 매칭되는 워크아웃 없음 (조회 결과가 비어있음)');
        return null;
      }

      console.log(`📊 [SamsungHealthService] 조회된 워크아웃 수: ${workouts.length}`);
      console.log('🔍 [SamsungHealthService] 조회된 워크아웃 샘플:', JSON.stringify(workouts.slice(0, 2), null, 2));
      ...
      const runningWorkouts = workouts.filter(workout => {
        ...
        if (!isRunning) {
          console.log('⚠️ [SamsungHealthService] 러닝이 아닌 워크아웃:', { 
      ...
      if (runningWorkouts.length === 0) {
        console.log('❌ [SamsungHealthService] 러닝 워크아웃 없음');
        return null;
      }

      console.log(`🏃 [SamsungHealthService] 러닝 워크아웃 수: ${runningWorkouts.length}`);
      ...
      if (!closestWorkout) {
        console.log('❌ [SamsungHealthService] 매칭되는 워크아웃 없음 (시간 비교 실패)');
        return null;
      }
      ...
      console.log('✅ [SamsungHealthService] 매칭되는 워크아웃 발견:', {
        시작시간: {
      ...
      // 워크아웃 데이터 추출 및 포맷팅
      console.log('🔍 [SamsungHealthService] closestWorkout 원본 데이터:', JSON.stringify(closestWorkout, null, 2));
      
      // 거리: 마일을 미터로 변환 (Samsung Health는 미터 단위로 반환하지만, HealthKit과 호환을 위해 마일로 변환된 것으로 가정)
      const distanceMiles = closestWorkout.distance || 0;
      const distanceMeters = distanceMiles * 1609.34; // 마일을 미터로 변환
      console.log('🔍 [SamsungHealthService] 거리:', { 마일: distanceMiles, 미터: distanceMeters });
      ...
      if (closestWorkout.duration) {
        ...
        console.log('🔍 [SamsungHealthService] duration 필드에서 추출:', durationSeconds);
        ...
        console.log('🔍 [SamsungHealthService] start/end로부터 계산된 duration:', durationSeconds, '초');
      }
      
      console.log('🔍 [SamsungHealthService] 최종 duration (초):', durationSeconds);
      ...
      if (durationSeconds > 0 && distanceMeters > 0) {
        ...
        console.log('🔍 [SamsungHealthService] 계산된 페이스:', paceFormatted);
      ...
      console.log('🔍 [SamsungHealthService] 포맷팅된 duration:', durationFormatted);
      ...
      // 이동경로 좌표 조회
      console.log('🔍 [SamsungHealthService] 이동경로 좌표 조회 시작');
      
      let routeCoordinates = [];
      const workoutId = closestWorkout.id || closestWorkout.uuid || closestWorkout.workoutId;
      
      if (workoutId && SamsungHealth.getWorkoutRouteSamples) {
        try {
          routeCoordinates = await SamsungHealth.getWorkoutRouteSamples({
            id: workoutId
          });
          console.log(`✅ [SamsungHealthService] 이동경로 좌표 ${routeCoordinates.length}개 조회됨`);
        } catch (error) {
          console.warn('⚠️ [SamsungHealthService] 이동경로 조회 실패:', error);
        }
      }
      
      // 폴백: 시간 범위 기반 조회
      if (!routeCoordinates || routeCoordinates.length === 0) {
        console.log('🔍 [SamsungHealthService] 시간 범위 기반 이동경로 조회로 폴백');
        routeCoordinates = await this.getRouteCoordinates(matchedWorkoutStartTime, workoutEndDate);
      }
      
      console.log('🔍 [SamsungHealthService] 이동경로 좌표 조회 완료:', routeCoordinates ? `${routeCoordinates.length}개` : '0개');
      ...
      console.log('✅ [SamsungHealthService] 워크아웃 데이터 추출 완료:', {
        결과: result,
        원본데이터: {
          거리: distanceMeters,
      ...
      console.error('❌ [SamsungHealthService] 운동기록 조회 실패:', {
        error: error,
