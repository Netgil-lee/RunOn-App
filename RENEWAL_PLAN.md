...
#### 1-1. 마커 표시 및 토글
- [ ] 모임 마커 GeoFirestore로 로드
  - `getEventsNearbyHybrid` 함수로 반경 3km 내 모임 검색
  - 종료된 모임(`status: 'ended'`) 제외
  - 하위 호환성 고려 (기존 모임 + 새 모임 모두 포함)
- [ ] 카페 마커 GeoFirestore로 로드
  - `getCafesNearby` 함수로 반경 700m 내 카페 검색
- [ ] 토글 버튼 구현 (러닝모임/러닝카페)
- [ ] 토글별 마커 필터링
- [ ] 새 모임 생성 시 `coordinates` 필드만 저장하도록 수정

#### 1-2. 클러스터링
- [ ] 카카오맵 MarkerClusterer 설정
- [ ] 클러스터링 구현 (모임/카페 마커 통합 클러스터링)
- [ ] 클러스터 클릭 시 Bottom Sheet 표시

#### 1-3. Bottom Sheet 기본 구조
- [ ] `@gorhom/bottom-sheet` 라이브러리 설치
- [ ] Bottom Sheet 기본 구조 구현
- [ ] 최초 진입 시 부분 확장 상태 구현
- [ ] 지도 클릭/드래그 시 Bottom Sheet 자동 축소
- [ ] 네비게이션 처리 (MapScreen으로 돌아올 때 초기화)

#### 1-4. Bottom Sheet - 러닝모임 모드
- [ ] 목록 표시 (`MeetingCard` 컴포넌트 재사용)
- [ ] 모임 상세 화면 표시 (`EventDetailScreen` 통합)
- [ ] 검색바 추가 (모임 제목, 모임 태그로 필터링)
...
#### 1-6. 검색 기능
- [ ] 지도 탭 검색바에 통합 검색 함수 연결
- [ ] 검색 결과 드롭다운 표시 (리스트 형식, 최대 5개 결과)
- [ ] 검색 결과 선택 후 지도 이동
- [ ] 검색 실패 처리 ("장소를 찾을 수 없습니다" 알림)
