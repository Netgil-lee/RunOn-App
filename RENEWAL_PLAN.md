...
- [x] 카페 마커 GeoFirestore로 로드
  - `getAllCafes` 함수로 모든 카페 조회 (반경 제한 없음) ✅
  - 프리미엄 색상 (#FF0073)으로 마커 표시 ✅
  - 마커 클릭 시 지도 이동, 확대, Bottom Sheet 확장 ✅
- [x] 토글 버튼 구현 (러닝모임/러닝카페) ✅
- [x] 토글별 마커 필터링 ✅
- [x] 토글 변경 시 지도 초기화 (현재 위치 또는 기본 위치로 이동) ✅
- [x] 인포윈도우 개선 ✅
  - 마커와 더 가깝게 배치 (yAnchor: 0.1, margin-top: -15px) ✅
  - 마커 클릭 시에만 표시 (자동 열림 방지) ✅
  - 필수 동작 실행 시 인포윈도우 유지 ✅
  - 마커 재생성 시 인포윈도우 복원 ✅
  - 카페 인포윈도우에 상호명 + 러닝인증 대표 혜택 텍스트 표시 ✅
...
- [x] 목록 표시 (카페 카드 컴포넌트) ✅
- [x] 카페 상세 정보 표시 ✅
  - 카페 상호명 + 소개 (상호명 바로 아래, 타이틀 없음) ✅
  - 이미지 캐러셀 (대표사진, 기본사진, 러닝인증할인혜택 이미지) ✅
  - 페이지 인디케이터 (3개 점, 현재 이미지 위치 표시) ✅
  - 러닝인증 혜택, 주소(타이틀 옆에 내용), 운영시간 표시 ✅
  - 타이틀 텍스트 색상 흰색으로 통일 ✅
- [x] 검색바 추가 (카페 상호명으로 필터링) ✅
- [x] 카페 클릭 시 최신 데이터 조회 (`getCafeById` 함수) ✅
  - 관리자 대시보드에서 수정한 내용 실시간 반영 ✅
...
- [x] 검색바 UI 개선
  - 검색바 높이 증가 (44 → 52) ✅
  - 검색바 모양 더 둥글게 (borderRadius: 8 → 26) ✅
  - 검색바 글씨 크기 및 돋보기 아이콘 크기 증가 (14 → 16, 20 → 24) ✅
  - 검색바 흰색 테두리 추가 (0.5px) ✅
  - 검색바와 현재 위치 버튼 사이 간격 추가 (gap: 8) ✅
- [x] 현재 위치 버튼 개선
  - 이미지 아이콘 사용 (locate_button.png) ✅
  - 배경색을 검색바와 동일하게 변경 ✅
  - 그림자 효과 추가 (은은한 그림자) ✅
- [x] 토글 UI 개선
  - 태그 UI와 동일한 스타일로 변경 (borderRadius: 20, 배경색: #1C3336) ✅
  - 크기 증가 (paddingVertical: 8 → 12) ✅
  - 비활성 상태 스타일 변경 (텍스트 흰색, 배경색 검색바와 동일) ✅
  - 그림자 효과 추가 ✅
- [x] 검색 전용 화면 구현
  - 검색바 포커스 시 전체 화면 검색 모드로 전환 ✅
  - 자동 검색 리스트 표시 ✅
  - 뒤로가기 아이콘으로 변경 (프라이머리 색, 설정화면과 동일한 UI) ✅
  - 키보드 엔터 키를 "검색"으로 변경 ✅
  - 검색 결과 선택 시 지도 이동 및 마커 표시 ✅
  - 검색 전용 화면 검색바에도 흰색 테두리 유지 ✅
- [x] 검색 모드 전환 애니메이션
  - 검색 모드 진입 시 현재 위치 버튼 사라지기 애니메이션 ✅
  - 검색바 확장 애니메이션 (현재 위치 버튼 공간까지 확장) ✅
  - 검색바 테두리 흰색 나타나기 애니메이션 ✅
  - 검색 결과 선택 시 역방향 애니메이션 ✅
- [x] Bottom Sheet 개선
  - 최대 확장 높이를 토글 버튼 아래로 제한 (topInset, maxDynamicContentSize 사용) ✅
  - 토글 버튼과 Bottom Sheet 사이 여백 확보 (12px) ✅
  - 배경색을 프로필 탭 카드 배경색으로 변경 (#171719) ✅
  - 확장 높이 조정 (90% → 60%) ✅
  - 마커 클릭 시 확장된 상태에서 Bottom Sheet 고정 (내부 스크롤만 가능) ✅
- [x] 마커 클릭 시 지도 동작 개선
  - 마커 클릭 시 마커 위치를 기준으로 지도 확대 기능 구현 (moveToMarkerAndZoom) ✅
  - 마커 위치로 지도 중심 이동 후 레벨 4로 확대 ✅
...
#### 2-1. 인증 및 권한 관리
- [x] Firebase Auth 인증 구현 (이메일/비밀번호)
  - [x] 로그인 화면 UI/UX 구현 (기존 사용자 관리 화면과 통합)
  - [x] 로그인 상태 유지 기능 구현
  - [x] 이메일 저장 기능 구현
- [x] Firestore `admins` 컬렉션 생성 및 관리자 UID 등록
  - [x] 기존 관리자 대시보드에 관리자 UID 등록 기능 추가
  - [x] 관리자 계정 생성 기능 추가
- [x] 관리자 권한 체크 기능 구현
  - [x] 로그인 시 한 번만 권한 체크
  - [x] 권한 없는 사용자 접근 시 에러 메시지 표시

#### 2-2. 카페 등록 폼
- [x] 러닝카페 등록 폼 UI 구현
  - [x] 업종 선택 필드 (카페, 음식점, 러닝샵)
  - [x] 요일별 운영시간 입력 필드 (TimePicker, 휴무일 체크박스)
  - [x] 인스타그램 링크 입력 필드
  - [x] 기본 정보 입력 필드 (상호명, 소개, 주소, 좌표)
  - [x] 기타 정보 입력 필드 (대표 메뉴, 러닝인증 혜택 텍스트)
- [x] 이미지 업로드 기능 구현 ✅
  - [x] 이미지 선택 UI 구현 ✅
  - [x] Firebase Storage 연동 ✅ (관리자 대시보드 백엔드 서버에서 구현)
  - [x] 파일 선택 지원 ✅
  - [x] 이미지 업로드 및 URL 저장 ✅
  - [x] 카페 사진 2장, 러닝인증 이미지 1장 업로드 UI ✅
  - [ ] 드래그 앤 드롭 지원 (선택 사항)
  - [ ] 이미지 리사이징 (Bottom Sheet 표시 사이즈) - 현재 원본 크기 사용
- [x] 지도에서 위치 선택 기능 구현
  - [x] Kakao Maps API (웹 SDK) 사용
  - [x] 지도 클릭 선택
  - [x] 주소 검색 기능 (Kakao Places API)
  - [x] 현재 위치 버튼
  - [x] 네이버지도 스타일 현재위치 마커
- [x] 폼 유효성 검사 구현
  - [x] 카페 상호명/소개 30자 제한
  - [x] 주소 필수 입력 검증
  - [x] 좌표 필수 입력 검증
  - [x] 이미지 업로드 검증 (개수, 필수 여부)
  - [ ] 인스타그램 URL 형식 검증
- [x] Firestore에 카페 데이터 저장 기능 구현 ✅
  - 관리자 대시보드 백엔드 서버에서 구현 완료 ✅
  - GeoFirestore 형식 (`g`, `l` 필드) 자동 생성 ✅
- [x] GeoFirestore `coordinates` 필드 자동 생성 ✅
...
#### 3-1. 모임 생성 위치 선택 변경
- [x] `screens/ScheduleScreen.js`에서 한강공원/강변 버튼 제거
- [x] 검색바 + 검색 결과 리스트로 변경
...
- [x] `screens/HomeScreen.js`에서 홈 탭 오버레이 가이드 삭제 ✅
  ...
  - 모임 탭 가이드는 유지 ✅
- [x] `screens/HomeScreen.js`에서 `HanRiverMap` 제거 ✅
- [x] 마이 대시보드 컴포넌트 생성 (`components/MyDashboard.js`) ✅
  - 자주 찾아가는 러닝카페 (마커 클릭 횟수 기준, 상위 3개, 2회 이상)
  - 자주 개설하는 모임장소 (모임 생성 횟수 기준, 상위 3개, 2회 이상)
  - 카페 카드: 대표 사진 + 카페명 + 방문 횟수
  - 장소 카드: 장소명 + 상세위치 + 개설 횟수 (아이콘 없음)
  - 빈 상태 UI 구현
  - 카드 클릭 시 지도탭 연동
- [x] 사용자 활동 데이터 수집 로직 구현 (`services/userActivityService.js`) ✅
  - 카페 방문 기록 저장 (MapScreen에서 카페 마커 클릭 시)
  - 모임 장소 기록 저장 (ScheduleScreen에서 모임 생성 완료 시)
  - Firestore `users/{userId}` 문서에 `frequentCafes`, `frequentMeetingLocations` 맵 구조로 저장
  - 삭제된 카페 자동 제외 로직 포함
- [x] 신규 입점 카페 목록 컴포넌트 생성 (`components/NewCafesList.js`) ✅
  - 배너/캐러셀 형태 UI (스와이프로 다음 카페 보기)
  - 최근 1개월 내 입점 카페 최대 10개 표시
  - 카페 정보: 대표 사진 + 카페명 + 위치 + 입점일
  - NEW 뱃지 표시
  - 페이지 인디케이터 (하단 점)
  - 클릭 시 지도탭 해당 카페로 이동
- [x] Firestore 데이터 구조 설계 및 구현 ✅
...
## 📊 현재 구현 상태 요약 (2026-01-25 업데이트)

### ✅ 완료된 주요 기능

#### Phase 0: 공통 인프라 구축
- ✅ GeoFirestore 설정 완료
- ✅ Kakao Places API 통합 완료
- ✅ 지도 탭 기본 구조 완료

#### Phase 1: 지도 탭 핵심 기능
- ✅ 모임/카페 마커 표시 및 토글 기능
- ✅ 클러스터링 구현
- ✅ Bottom Sheet (모임/카페 모드)
- ✅ 검색 기능 (Firestore 우선, Kakao Places API)
- ✅ 인포윈도우 개선 (마커 클릭 시에만 표시, 프리미엄 색상)
- ✅ 카페 마커 프리미엄 색상 (#FF0073) 적용
- ✅ 카페 마커 반경 제한 제거 (모든 카페 표시)
- ✅ 토글 변경 시 지도 초기화

#### Phase 2: 관리자 대시보드
- ✅ 인증 및 권한 관리
- ✅ 카페 등록 폼 (Firebase Storage 연동 포함)
- ✅ 카페 관리 기능 (조회, 수정, 삭제)
- ✅ GeoFirestore 형식 자동 생성

#### Phase 3: 모임 생성 및 홈화면 리뉴얼
- ✅ 모임 생성 위치 선택 변경 (Kakao Places API 통합)
- ✅ 모임설명(description) 필드 추가
- ✅ 공유 이미지 place 입력 기능
- ✅ 홈화면 리뉴얼 완료
  - ✅ 마이 대시보드 (자주 찾는 카페, 자주 개설하는 장소)
  - ✅ 신규 입점 카페 목록 (캐러셀 형태)

#### 카페 상세 정보 기능
- ✅ 카페 상세 정보 표시 (이미지 캐러셀, 페이지 인디케이터)
- ✅ 카페 클릭 시 최신 데이터 조회 (`getCafeById`)
- ✅ 레이아웃 개선 (소개 타이틀 제거, 주소 타이틀 옆에 내용)
...
- [ ] 지연 로딩 구현
- [ ] 마커 렌더링 최적화
- [ ] 통합 테스트 (지도 탭 전체 플로우, 검색 기능, 클러스터링 등)
...
- **Phase 0 (공통 인프라)**: 100% ✅
- **Phase 1 (지도 탭 핵심 기능)**: 약 95% ✅ (모임카드 더보기 기능 제외)
- **Phase 2 (관리자 대시보드)**: 약 90% ✅ (이미지 리사이징, 드래그앤드롭 제외)
- **Phase 3 (모임 생성 및 홈화면 리뉴얼)**: 100% ✅
- **Phase 4 (성능 최적화 및 테스트)**: 0% (추후 진행)
