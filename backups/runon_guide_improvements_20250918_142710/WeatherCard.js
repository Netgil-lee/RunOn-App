...
const WeatherCard = ({ onWeatherDataUpdate, isRefreshing = false }) => {
  ...
  const fetchWeather = async (coords = null) => {
    try {
      ...
      // 현재 날씨, 예보 데이터, 미세먼지 데이터를 병렬로 가져오기
      const [currentResponse, forecastResponse, airQualityData] = await Promise.all([
        fetch(
        ...
        airQualityService.fetchAirQuality(latitude, longitude)
      ]);
      ...
      // 미세먼지 알림 확인
      const airQualityAlerts = airQualityService.checkAirQualityAlerts(airQualityData);
