...
const HomeScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    if (isLoading || hasError || !userProfile || !guideStates) {
      return;
    }
    ...
    if (userProfile.onboardingCompleted && !guideStates.homeGuideCompleted) {
      ...
      // 동적 위치 계산을 위해 스크롤 후 측정하므로 여기서는 초기 설정만 수행
      
      // 한강 지도 영역으로 자동 스크롤 후 가이드 시작
      setTimeout(() => {
        scrollToHanRiverMap();
        
        // 스크롤 완료 후 동적 위치 계산 및 가이드 시작
        setTimeout(() => {
          // 가이드 시작 (동적 위치 계산은 scrollToHanRiverMap에서 처리됨)
          setTimeout(() => {
            startGuide('home');
          }, 500); // 동적 계산 완료 후 추가 지연
        }, 1000);
      }, 500);
    }
  }, [isLoading, hasError, userProfile, guideStates]);
    ...
    <View style={styles.container}>
      ...
      {__DEV__ && (
        <View style={styles.devToolbar}>
          <TouchableOpacity 
            style={styles.devToolbarButton} 
            onPress={handleResetHomeGuide}
          >
            <Text style={styles.devToolbarButtonText}>홈</Text>
          </TouchableOpacity>
          <TouchableOpacity 
            style={styles.devToolbarButton} 
            onPress={handleResetMeetingGuide}
          >
            <Text style={styles.devToolbarButtonText}>모임</Text>
          </TouchableOpacity>
          <TouchableOpacity 
            style={[styles.devToolbarButton, styles.devToolbarButtonDanger]} 
            onPress={handleResetAllGuides}
          >
            <Text style={styles.devToolbarButtonText}>전체</Text>
          </TouchableOpacity>
        </View>
      )}
