...
const ChatScreen = ({ route, navigation }) => {
        ...
        try {
          if (messageData.senderId) {
            ...
            if (userProfile) {
              ...
              senderName = userProfile.profile?.nickname || userProfile.displayName || messageData.sender || '익명';
            }
          }
        } catch (error) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (userProfile) {
        senderName = userProfile.profile?.nickname || userProfile.displayName || user?.email?.split('@')[0] || '나';
        senderProfileImage = userProfile.profileImage || null;
      }
    } catch (error) {
      ...
      return (
        <View style={[styles.otherMessageWrapper, item.isConsecutive ? styles.otherMessageWrapperTight : null]}>
          ...
          <View style={styles.messageContentContainer}>
            {/* 발신자 이름 */}
            {showAvatarAndName ? (
              <Text style={styles.senderNameKakao}>{item.sender}</Text>
            ) : null}
