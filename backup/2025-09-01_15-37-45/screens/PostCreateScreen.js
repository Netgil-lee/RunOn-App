    ...
    if (isPostValid()) {
      try {
        ...
        try {
          ...
          authorName = userProfile?.profile?.nickname || userProfile?.displayName || user?.displayName || user?.email?.split('@')[0] || '사용자';
        } catch (error) {
        ...
        const newPost = {
          ...
          authorProfile: {
            ...
            profileImage: user?.photoURL || userProfile?.profileImage || null
          },
          likes: [],
          comments: []
        };
