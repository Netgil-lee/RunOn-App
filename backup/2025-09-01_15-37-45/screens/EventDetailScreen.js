...
const EventDetailScreen = ({ route, navigation }) => {
  const { event: rawEvent, isJoined = false, currentScreen, isCreatedByMe: routeIsCreatedByMe, returnToScreen } = route.params;
  const [isJoinedState, setIsJoinedState] = useState(isJoined);
  const [currentUserProfile, setCurrentUserProfile] = useState(null);
  const { user } = useAuth();
  const { endEvent, joinEvent, leaveEvent } = useEvents();
  ...
  // 디버깅: 버튼 상태 확인
  console.log('🔍 EventDetailScreen - 버튼 상태 확인:', {
    routeIsCreatedByMe,
    userUid: user?.uid,
        ...
        const participantsData = await Promise.all(
          event.participants.map(async (participantId, index) => {
            try {
              ...
              console.log(`🔍 EventDetailScreen - 참여자 ${index} 프로필 이미지 확인:`, {
                participantId,
                isHost,
                photoURL: userProfile?.photoURL,
                profileImage: userProfile?.profileImage,
                profileProfileImage: userProfile?.profile?.profileImage,
                finalProfileImage: profileImage
              });
              
              return {
                id: `participant_${index}`,
                name: isHost ? hostName : (userProfile?.profile?.nickname || userProfile?.displayName || '익명'),
            ...
            <View style={styles.participantAvatar}>
              {participant.profileImage && !participant.profileImage.startsWith('file://') ? (
                <Image 
                  ...
                  onError={(error) => {
                    console.log('❌ EventDetailScreen - 참여자 이미지 로드 실패:', {
                  ...
                  onLoad={() => {
                    console.log('✅ EventDetailScreen - 참여자 이미지 로드 성공:', {
                      participantId: participant.userId,
                      profileImage: participant.profileImage
                    });
                  }}
