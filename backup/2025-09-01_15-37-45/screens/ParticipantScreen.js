...
const ParticipantScreen = ({ route, navigation }) => {
  ...
  // 디버깅: 참여자 데이터 확인
  console.log('🔍 ParticipantScreen - 참여자 데이터:', {
    participant,
        ...
        <View style={styles.profileCard}>
          <View style={styles.profileRow}>
            <View style={styles.profileImageWrap}>
              {(() => {
                console.log('🔍 ParticipantScreen - 프로필 이미지 렌더링:', {
                  hasParticipant: !!participant,
                  profileImage: participant?.profileImage,
                  willShowImage: !!(participant && participant.profileImage)
                });
                
                if (participant && participant.profileImage && !participant.profileImage.startsWith('file://')) {
                  ...
                  return (
                    <Image 
                      ...
                      onError={(error) => {
                        console.log('❌ ParticipantScreen - 이미지 로드 실패:', error.nativeEvent);
                      }}
                      onLoad={() => {
                        console.log('✅ ParticipantScreen - 이미지 로드 성공:', participant.profileImage);
                      }}
                    />
                  );
                } else {
                  // 로컬 파일이거나 이미지가 없는 경우 기본 이미지 표시
                  return (
