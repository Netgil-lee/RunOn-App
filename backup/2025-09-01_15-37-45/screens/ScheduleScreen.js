...
const ScheduleScreen = ({ navigation, route }) => {
  ...
  const handleEventPress = (event, currentScreen) => {
    ...
    // 디버깅: 모임 클릭 정보
    console.log('🔍 ScheduleScreen - 모임 클릭 정보:', {
      currentScreen,
      eventId: event.id,
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        {/* 디버깅용 로그 */}
        {console.log('🔍 ScheduleScreen - 종료된 모임 카드 렌더링:', {
          endedEventsLength: endedEvents.length,
          endedEvents: endedEvents.map(e => ({ id: e.id, title: e.title, date: e.date, time: e.time })),
          hasRatingNotification: hasRatingNotificationForEndedEventsOption()
        })}
...
const ScheduleCard = ({ event, onEdit, onDelete, onPress, isCreatedByMe = false, showOrganizerInfo = false, cardIndex, showJoinButton = true, isEnded = false, hasRatingNotification = false, hasMeetingNotification = false }) => {
  ...
  // 디버깅을 위한 로그
  console.log('🔍 ScheduleCard - 모임 데이터:', {
